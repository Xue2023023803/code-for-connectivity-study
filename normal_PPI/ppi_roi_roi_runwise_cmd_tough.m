%% ppi_roi_roi_runwise_fromOriginal_cmd.m
...
% 思路：
%   - 在 ROI(L/R) 空间中，先对每个 ROI-pair (i,j) 在 run 维度上做固定效应平均
%   - 在 base ROI 空间中，对每个 base-pair (B1,B2)：
%       * 先取对应的 L(B1)->L(B2) 和 R(B1)->R(B2) 两条 ROI 边
%       * 各自在 run 维度上平均，得到 β_L_fix、β_R_fix
%       * bilateral β = mean([β_L_fix, β_R_fix] 忽略 NaN)
...
% 使用：
%   - 边强度：results_fix.(cond).beta_PPI_bilat (跨 run + L/R 固定效应平均后的 β)
%   - 统计显著性：基于 run-wise beta_PPI (results.(cond).beta_PPI) 中
%       L(B1)->L(B2) 和 R(B1)->R(B2) 的所有 run-wise β，合并作为重复观测，
%       在 "run × hemi" 维度上做 t 检验 vs 0；
