...
% 被试与路径（此脚本特化为旧实验 sub-02）
cfg.subj       = 'sub-02';
cfg.prf_ses    = 'ses-01';  % 旧实验 Gray/Original 合并的会话名

% 旧实验 mrVista 工程根（Gray/Original 在这里）
cfg.root_prf   = '/home/xue/daifenpei/sub-02/results1';

% anat + 输出 derivatives 的根（ses-03 在这里）
cfg.root_bids  = '/home/xue/daifenpei/sub-02/results3';
...
% 输出目录（放在 results3/derivatives 下）
...
for i = 1:nROI
    ...
    if h == 'L'
        roiHemi{i} = 'L';
        roiBase{i} = rn(2:end);  % 去掉首字母，保留后面的 base 名（例如 'nf','npc1'）
    elseif h == 'R'
        roiHemi{i} = 'R';
        roiBase{i} = rn(2:end);
    else
        % 没有 L/R 前缀，就当作无半球标签的 ROI
        roiHemi{i} = '';
        roiBase{i} = rn;
    end
end
...
% 旧实验 sub-02 的约定：
%   Gray Scan19–23: ses-05, run01–05
%   Gray Scan24–27: ses-06, run01–04
%   Gray Scan28–36: ses-07, run01–09
% meta.runIdx_connect/unconnect 只用 Scan19–36，其余 scan 对 PPI 不参与

runMeta = struct('scanIdx', cell(1,nScan), ...
                 'scanNum', [], 'sessionIdx', [], 'cond', [], ...
                 'runInSes', [], 'sesLabel', [], 'runLabel', []);

fprintf('\n[Step 1.1] 利用固定映射构造 runMeta（Gray Scan → BIDS ses/run）...\n');

for i = 1:nScan
    sNum   = scanInfo.scanNums(i);   % Gray Scan 号
    ...
    if sNum >= 19 && sNum <= 23
        sesIdx   = 5;
        runInSes = sNum - 18;   % 1..5
    elseif sNum >= 24 && sNum <= 27
        sesIdx   = 6;
        runInSes = sNum - 23;   % 1..4
    elseif sNum >= 28 && sNum <= 36
        sesIdx   = 7;
        runInSes = sNum - 27;   % 1..9
    else
        % 旧实验 PPI 不使用 Scan1–18，仅做占位
        fprintf('    [Warn] Gray Scan%02d 不在 19–36 范围内（旧实验 PPI 未使用），sessionIdx/runInSes 置为 NaN。\n', sNum);
        runMeta(i) = rm;
        continue;
    end
    ...
    rm.sesLabel   = sprintf('ses-%02d', sesIdx);    % 'ses-05' / 'ses-06' / 'ses-07'
...
for uu = used_idx(:)'
    ...
    assert(~isnan(rm_chk.runInSes), ...
        'scanIdx=%d (scanNum=%d) 被用于 PPI，但未在 19–36 映射中找到 sessionIdx/runInSes。', ...
        uu, rm_chk.scanNum);
end
fprintf('    [Check] 所有用于 PPI 的 runIdx_connect/unconnect 均成功映射到 ses-05/06/07 & run01–09。\n');
...
% WM / CSF 概率图路径（T1 space，固定在 results3/ses-03）
...
for ci = 1:numel(condLevels)
    ...
    for k = 1:nRun_cond
        ...
        % 不同 session 的 func 分别在 results5/6/7 下
        sesFuncRoot = get_func_root_for_session(rm.sesLabel);
        sesDir      = fullfile(sesFuncRoot, cfg.subj, rm.sesLabel, 'func');

        % 旧实验 BIDS 命名：sub-02_ses-0X_task-runYY_...
        baseStub= sprintf('%s_%s_task-run%02d', ...
                          cfg.subj, rm.sesLabel, rm.runInSes);
...
for ci = 1:numel(condLevels)
    ...
    % ---- 统计每个 ROI-pair 实际参与的 run 个数（非 NaN 数量，ROI 级）----
    ...
    % ---- (2) base ROI 空间：先对 L 和 R 分别跨 run 平均，再做 L/R 合并 ----
    beta_PPI_bilat = nan(nBase, nBase);
    Nr_bilat       = zeros(nBase, nBase);  % 用于记录参与 L/R 的非 NaN 数量 (总数)
    ...
    for b1 = 1:nBase
        ...
        for b2 = 1:nBase
            ...
            % L 半球：L(B1)->L(B2)
            b_L_runs = squeeze(beta_PPI(iL, jL, :));   % [nRun_cond × 1]
    ...
    results_fix.(condLabel).beta_PPI_bilat     = beta_PPI_bilat;  % base ROI × base ROI
...
out_name_ffx = fullfile(cfg.out_dir, ...
    sprintf('%s_ppi_roi_roi_FFX_fromOriginal_bilateral.mat', cfg.subj));
...
for ci = 1:numel(condLevels_fix)
    ...
    % bilateral 固定效应后的有向 β 矩阵（base ROI 空间）
    ...
    beta_fix_bilat = results_fix.(condLabel).beta_PPI_bilat;  % [nBase × nBase]

    % run-wise beta（ROI 空间，用于构造 L/R × run 的重复观测）
    beta_run = [];
    if isfield(results, condLabel) && isfield(results.(condLabel), 'beta_PPI')
        beta_run = results.(condLabel).beta_PPI;          % [nROI × nROI × nRun_cond]
    ...
    % ---------- 先在 "run × hemi" 维度上做 t 检验：每个 base ROI-pair 一组 β ----------
    p_bilat_dir = NaN(nBase, nBase);   % base(B1)->base(B2) 的 p 值
    ...
    if ~isempty(beta_run)
        for b1 = 1:nBase
            ...
            for b2 = 1:nBase
                ...
                b_L_runs = squeeze(beta_run(iL, jL, :));   % [nRun_cond × 1]
    ...
    for i = 1:nBase
        for j = i+1:nBase
            % β：对 B1->B2 和 B2->B1 的 bilateral β 取平均
...
function root = get_func_root_for_session(sesLabel)
% 根据 session label 选择对应的 resultsX 作为 func 根目录
switch sesLabel
    case 'ses-05'
        root = '/home/xue/daifenpei/sub-02/results5';
    case 'ses-06'
        root = '/home/xue/daifenpei/sub-02/results6';
    case 'ses-07'
        root = '/home/xue/daifenpei/sub-02/results7';
    otherwise
        error('未为 %s 定义 func 根目录（resultsX）。', sesLabel);
end
end
